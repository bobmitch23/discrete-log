...
def solve(gen, power, prime):
    ...
    for i in range(0, len(prime_factor_multiplcities)):
        ...
        for j in range(0, (prime_factor_multiplcities[i][1])):
            y = pow(power, (prime1//((prime_factor_multiplcities[i][0])**(j+1))), prime)
            for k in range(0, (prime_factor_multiplcities[i][0])):
                if pow(x, k, prime) == y:
                    u += k*(prime_factor_multiplcities[i][0]**j)
                    break          
            power = (power * pow(genInv, (prime_factor_multiplcities[i][0]**j)*k, prime)) % prime
        finalList.append(u)
        u = 0
        power = power_copy
    factors = []
    ...
    if pow(gen, finalNum, prime) == power:
